...
public class AppCore {
    ...
    public static ResponseEntity<String> handleRequestFromAnotherBank(Response response, Request req, ForeignBank foreignBank,
        ...
        ResponseEntity<String> responseEntity = foreignBank.sendRequest(req, response, account, bank);
        if (gson.fromJson(responseEntity.getBody(), ForeignBankResponse.class).isRequestSuccessful()) {
            em.getTransaction().commit();
        } else {
            em.getTransaction().rollback();
        }
    ...
    private static Response checkBalance(EntityManager em, Request req) {
        ...
        if (!userExists(em, req.getUserID())) {
            ...
            if (acc == null) {
                ...
                response.setRequestSuccessful(false);
            } else if (!req.getUserID().equals(acc.getUser().getId())) {
                response.setResponseMessage("Can't check money operation: authorization failure");
                response.setRequestSuccessful(false);
            } else {
                response.setRequestSuccessful(true);
    ...
    private static Response performMoneyOperation(EntityManager em, Request req, Account acc) {
        ...
        if (req.getReqType().equals(RequestType.ADD_FUNDS)) {
            ...
            if (req.getUserID() == null) {
                response.setResponseMessage("Can't perform money operation: authorization failure");
                response.setRequestSuccessful(false);
            } else if (!req.getUserID().equals(acc.getUser().getId())) {
                response.setResponseMessage("Can't perform money operation: authorization failure");
                response.setRequestSuccessful(false);
            } else if ((balance - req.getMoneyToMove()) < 0) {
